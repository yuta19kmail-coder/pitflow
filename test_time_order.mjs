...
console.log('\n── ① 並びの値（いちばん遅くなり得る時刻） ──');
{
  const L = ['09:00','09:15','09:30','朝一','09:45','10:00','09:00-10:00','AM','12:30','12:59','13:00','お昼','16:00','18:30','夕方','PM','決まり次第','レッカー','鍵ポスト','勝手に取る','未定','','9時以降'];
  const order = await p.evaluate(list =>
    list.map(v => [v, pitTimeMin(v)]).sort((a, b) => a[1] - b[1]).map(x => x[0]), L);
  /* 🔴 v1.105.0（ゆうた変更）AM の終わりは 11:59 になったので、**AM は 12:00 台のどの車よりも前**。 */
  const want = ['09:00','09:15','09:30','朝一','09:45','10:00','09:00-10:00','AM','12:30','12:59','13:00','お昼','16:00','18:30','夕方','PM','決まり次第','レッカー','鍵ポスト','勝手に取る','未定','','9時以降'];
  ...
  ok('🔴 AM は 11時台のいちばん最後（12:00 より前・v1.105.0）', v.AM > 719 && v.AM < 720, v);
  ...
  ok('端数は1分をまたがない（AM も 12:00 を越えない）', Math.floor(v.AM) === 719, v);
}
...
console.log('\n── ② 枠（◯時台）も同じ物差し ──');
{
  ...
  ok('🔴 AM は 11時の枠（v1.105.0 で 12時の枠から移った・12時台に被せない）', h.AM === '11', h);
...
console.log('\n── ④ 予約カレンダー（日）＝枠の中も時間順・AM は12時の枠 ──');
{
  ...
  /* 🔴 v1.105.0（ゆうた変更）AM は **11時の枠**。12時の枠には 12:30 だけが残る。 */
  ok('🔴 AM は 11時の枠に入る', JSON.stringify(at('11:00')) === JSON.stringify(['TAM']), at('11:00'));
  ok('🔴 12時の枠に AM はもう居ない（12:30 だけ）', JSON.stringify(at('12:00')) === JSON.stringify(['T12_30']), at('12:00'));
