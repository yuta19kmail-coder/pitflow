...
/* ================================================================
   🚗 v2.12.0 「仮登録車両」はナンバーではない（＝整備ソフトが番号の無い車に入れる文言）。
   ----------------------------------------------------------------
   🔴 ナンバーとして使うと、その文言の車が**全部おなじ1台に吸い込まれる**。
      だから `pitIsRealPlate` が false を返す＝🟡「人が見て確かめて」側に残る。
   ⚠ このテストが見たいのは「Qごと」の仕組みのほう。
      だから**人がやるのと同じように印を押して片づけてから**、本題に入る。
   ================================================================ */
const 仮登録 = await p.evaluate(async () => {
  const U = window._insp.q; let n = 0;
  (U.groups || []).forEach(g => ((g.res && g.res.結びついた) || []).forEach(p => {
    if (window.pitIsRealPlate(p.soft && p.soft.ナンバー)) return;
    n++;
    (window.pitQFixKinds(p) || []).concat(window.pitQKeepKinds(p) || [])
      .forEach(k => window.pitQMark(k.kind, p.soft, p.pit, true));
  }));
  await new Promise(r => setTimeout(r, 600));
  return n;
});
console.log('\n── 🚗 「仮登録車両」はナンバーとして使わない ──');
ok('この月のPDFに3件ある（あけぼの自動車ほか）', 仮登録 === 3, 仮登録);
await p.evaluate(() => { window.pitQClearForMonth('2026-08');
                         const el = document.getElementById('qfile'); if (el) el.value = ''; });
await p.setInputFiles('#qfile', PDF);
await p.waitForFunction('window._insp.q.res && !window._insp.q.busy', null, { timeout: 60000 });
await p.waitForTimeout(400);
