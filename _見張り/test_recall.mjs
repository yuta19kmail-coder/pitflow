...
console.log('\n── 🔤 文字のならし方をマスター検索と揃える ──');
{
  ...
  /* 🔵 2026-08-30（v2.39.0）**当たるようになった。**
     旧字・異体字を探せるようにする時、`search.js` の物差しに `normalize('NFKC')` を足した。
     そのおまけで**全角ハイフン（－）も半角に寄る**ようになった。
     ⚠ 直したのは**物差し1本（`pitSearchNorm`）**なので、呼び出しもマスター検索も**同時に**効いている
        ＝ここだけ直して食い違う、という形にはなっていない（このメモは前の版の申し送り）。 */
  const r4 = await recall('０９０－１１１１－１００３');
  ok('🔵 全角ハイフン混じりでも当たる（物差しを1本直したので両方に効く）', r4.n >= 1, r4.n);
}
