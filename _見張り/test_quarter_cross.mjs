/* PitFlow v2.8.0 ── Qをまたいだ車を、二度言わない
   ...
   ◎この試験がやること
     ...
     🔴 ④ v2.101.0 から：照合できた（別のQ確定）行は緑のOK・変更ボタンなし／推しただけの行は赤で数える＋ヒント
...
console.log('\n── 🔗 ②PDFが片方のQしか無い（別の回に走らせる） ──');
{
  ...
  /* 🔴 v2.101.0 ゆうた指定：7月のPDFで照合できていない（推しただけ）なら**赤で数える**。8月のPDFで照合できれば消える */
  ok('🔴 照合できていない行は残り件数に数える（+1）', jul[0].残り === jul[0].非OK結び + 1, { 残り:jul[0].残り, 非OK結び:jul[0].非OK結び });
...
console.log('\n── 🃏 ④ 照合できた行は緑のOK・推しただけの行は赤＋ヒント（v2.101.0） ──');
{
  /* 🗣 ゆうた 2026-09-13「伝票がない場合はアカでOK」「Q3で照合してあるからOK的なニュアンスを強く、変更できるボタンは無くして」 */
  const mk = (row) => ({
    ...
    検算: { 合う:true }, まとめ返車: [],
    直すもの: { 期間の外: [], 月またぎ: [], Qまたぎ: [], 売上日ちがい: [], 担当ちがい: [], 金額ちがい: [],
               整備ソフトだけ: [], PitFlowだけ: [row] }
  });
  const base = { 数える日:'2026-08-05', 予約番号:'R-2401', ナンバー:'習志野 500 あ 11-11',
                 お客様:'Ａさん', 金額:100000, フロント:'蓮沼', カードid:'card-A', 車種:'ノア', 車体番号:'ZRR80-1234567',
                 別のQ:'伝票は 8月 第2クォーター（2026-08-10・D-A）にあります' };
  const render = (saved, tab) => p.evaluate(({ saved, tab }) => {
    const U = (window._insp = window._insp || {}); U.q = U.q || {};
    Object.assign(U.q, { res:null, pdf:null, saved, savedId:'qrun', tab, savedTab:'PitFlowだけ', list:[], groups:[] });
    return window.pitQuarterHtml();
  }, { saved, tab });

  const h1 = await render(mk(base), 'data');
  ok('🔴🔴 推しただけの行は赤（gone）', /class="q-c gone[ "]/.test(h1), (h1.match(/class="q-c [^"]*"/g) || []).slice(0, 3));
  ok('🔴 黄では出さない', !/gone-y/.test(h1));
  ok('見出しは「フロントマンに伝票が無い」', h1.includes('フロントマンに伝票が無い'));
  ok('🔴 どのQにありそうかは「ヒント」として出る', h1.includes('ヒント：') && h1.includes('第2クォーター') && h1.includes('D-A'));

  const h2 = await render(mk(Object.assign({}, base, { 別のQ確定: true, 別のQ先: { label: '8月 第2クォーター', from: '2026-08-08', 売上日: '2026-08-10', 伝票: 'D-A' } })), 'ok');
  const c2 = h2.split('q-c okq')[1] || '';
  ok('🔴🔴 照合できた行は緑（okq）', /class="q-c okq[ "]/.test(h2), (h2.match(/class="q-c [^"]*"/g) || []).slice(0, 3));
  ok('🔴🔴 「Q2 で照合済み…直すところはありません」と言い切る', c2.includes('Q2 で照合済み') && c2.includes('直すところはありません'), c2.slice(0, 400));
  ok('🔴🔴 変更できるボタンを出さない（カードを開く・取り消す・伝票を立てる）', !/カードを開く|取り消す|伝票を立て/.test(c2), c2.slice(0, 600));
  ok('🔴 赤でも黄でもない', !/class="q-c gone/.test(h2));
}
