...
console.log('\n── ③ ナンバー空欄化スクリプトの中身（読むだけの見張り） ──');
{
  ...
  /* ⚠ v2.51.0 の片づけで、1回きりの取り込み・突合のスクリプトは
     `_資料/05_取り込み・突合のデータ/` に移した（PitFlow 直下は空にした）。 */
  const _fxp = ['../_資料/05_取り込み・突合のデータ/顧客控え_ナンバー0を空にする.js',
                '../顧客控え_ナンバー0を空にする.js',
                '../out6/顧客控え_ナンバー0を空にする.js'].find(x => fs.existsSync(x));
