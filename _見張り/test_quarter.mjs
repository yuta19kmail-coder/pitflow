...
console.log('\n── ⑦ 画面（並べるだけ・判定を書き写していない） ──');
{
  const r = await p.evaluate(([soft, pit]) => {
    ...
    return {
      ...
      ai: /AI に渡すもの/.test(t2),
      /* 🔢 v2.77.0 全体の金額は出さない（ゆうた指定） */
      箱の金額: body.querySelectorAll('.q-grb-v').length + body.querySelectorAll('.q-pq-v').length
              + body.querySelectorAll('.q-done-v').length,
      箱に円: Array.from(body.querySelectorAll('.q-grb')).some(x => /円/.test(x.textContent || '')),
      残りに円: /円/.test((body.querySelector('.q-diff') || {}).textContent || ''),
      合計の金額: body.querySelectorAll('.q-sum .q-y').length,
      詳細に金額: /金額/.test((body.querySelector('.q-cards .q-c') || {}).textContent || '')
    };
  }, [SOFT, PIT]);
  ...
  /* ================================================================
     🔢 v2.77.0（ゆうた指定 2026-09-07）**全体は件数だけ。まとめた金額は出さない。**
     🗣「あんまりQを正確な金額で管理してないから、金額出されるとかえって混乱するかも。
     　　金額は各詳細でいくら違うよってのは出ていいけど、**全体として件数のみで対応したい**」
     ⚠ 消したのは**画面に出す所だけ**。数え方も検算も保存する中身も変えていないので、
     　 上の「合計が画面に出る」「検算の結果が画面に出る」は**今までどおり緑**でなければならない。
     ================================================================ */
  ok('🔢🔢 まとめた金額の枠が1つも無い（Qの箱・グループの箱・チェック済み）', r.箱の金額 === 0, r.箱の金額);
  ok('🔢 グループの箱に「円」が出ない', r.箱に円 === false, r.箱に円);
  ok('🔢 「まだ合っていない」は件数だけ（円が出ない）', r.残りに円 === false, r.残りに円);
  ok('🟢 上の2枚（フロントマン／PitFlow）の金額は残す（ゆうた確定）', r.合計の金額 === 2, r.合計の金額);
  ok('🟢 各詳細のカードには金額が出る（「詳細ではいくら違うか」は要る）', r.詳細に金額 === true, r.詳細に金額);
  /* ⚠ v2.3.0 ③は「渡すものの一覧」ではなく **入れたPDFの中身を見る画面**に入れ替えた。
     この材料（手で書き出したJSON）は明細を持っていないので、③は「PDFを入れてください」と言う。
     ③そのものは ⑧ で見張っている。 */
}
...
console.log('\n── ⑧🤖 ③伝票の中身を見る（v2.3.0・鍵は画面に置かない） ──');
/* 🔴🔴 v2.3.0（ゆうた 2026-08-24）③は**入れたPDFの中身**を見る画面に入れ替えた。
   🗣「既存の全体を見るのではなく、付け合わせようにアップロードしたPDFの中身の確認に全ぶりする」
   🔴 **AIを走らせなくても並ぶ**（覚えと照らすところにお金はかからない）。ここが肝。 */
{
  const r = await p.evaluate(() => {
    /* 明細つきの伝票を1枚だけ置く＝数が読める */
    window._insp.q = Object.assign(window._insp.q || {}, {
      groups: null, gi: 0,
      soft: [{ 売上日:'2026-08-04', 伝票:'0001', ナンバー:'船橋 300 あ 1111', 顧客名:'あ 一郎',
               車種:'ミニ', 金額:100000,
               明細:[{ 種:'見出し', 名:'【一般整備】' },
                     { 種:'作業', 名:'エンジン・オイル交換', 金額:3000, 原価:0 },
                     /* うちのふだんは 45〜60%。90% は外れる */
    ...
    window._insp.ai = { busy:'', err:'', 見立て:null, at:'', usage:null, tab:'掛け率' };
    const h1 = pitAiHtml();
    window.__adm = true;
    const h2 = pitAiHtml();
    const look = window.pitCostLook(window._insp.q.soft[0]);
    window.PIT_CLOUD = before;
    return {
      並ぶ: /掛け率がちがう/.test(h1) && /初めて見る品名/.test(h1),
      AIなしで出る: /AIを使わなくても出ています/.test(h1),
      箱: (h1.match(/<button class="ai-grb/g) || []).length,
      ボタン: /AIに見立てを付けてもらう/.test(h2),
      書き出し: /送るものを書き出す/.test(h2),
      断定しない: /まちがいとは言っていません/.test(h1) && /決めるのは人です/.test(h1),
      種: look.map(x => x.種),
      掛け率: look.filter(x => x.種 === '掛け率').map(x => ({ 名:x.名, いま:x.いま, ふだん:x.ふだん })),
      初めて: look.filter(x => x.種 === '初めて').map(x => x.名)
    };
  });
  ok('🔴🔴 AIを走らせなくても、気になる行が並ぶ（お金がかからない）', r.並ぶ === true, r);
  ok('🔴 そのことを画面にも書いてある', r.AIなしで出る === true, r);
  ok('🗂 入り口は5つ（掛け率／売価／いつものが無い／初めて／両方使う）', r.箱 === 5, r.箱);
  ok('🔴 うちのふだんから外れた掛け率を拾う',
     r.掛け率.length === 1 && r.掛け率[0].名 === 'オイルフィルター' && r.掛け率[0].いま === 90, r.掛け率);
  ok('🔴 覚えに無い品名は「初めて見る品名」として出す（黙って捨てない）',
     r.初めて.indexOf('ぜんぜん知らない部品') >= 0, r.初めて);
  ok('🔴 管理の人にはAIのボタンが出る', r.ボタン === true, r);
  ok('🔴 何を送るかは、いつでも自分で書き出せる', r.書き出し === true, r);
  ok('🔴🔴 断定しないと書いてある（最後は紙と人）', r.断定しない === true, r);
}

console.log('\n── ⑧-2📚 会社の覚え（過去の実績から作ったもの） ──');
{
  const r = await p.evaluate(() => {
    const B = window.PIT_COST_BOOK;
    const g = (n) => window.pitCostOf(n);
    return {
      伝票: B.伝票, 品: Object.keys(B.品).length,
      産廃: g('産業廃棄物処理費用'),
      Ｏ全角: g('Ｏリング') && g('Ｏリング').n,      /* 全角でも半角の覚えに当たる */
      O半角: g('Oリング') && g('Oリング').n,
      切れ: g('産業廃棄物処理費用・ショートパー') && g('産業廃棄物処理費用・ショートパー').n,
      無い: g('こんな品名は無いはず'),
      組: !!window.pitCostSetOf('エンジン・オイル交換')
    };
  });
  ok('📚 過去の伝票から作ってある（864枚）', r.伝票 === 864, r.伝票);
  ok('📚 品名ごとの目安を持っている', r.品 > 200, r.品);
  ok('🔴🔴 産廃は原価1円＝掛け率ほぼ0（線ひとつでは切れない山の1つ）',
     r.産廃 && r.産廃.率[2] <= 0.01, r.産廃);
  ok('🔴 全角の「Ｏリング」も半角の覚えに当たる', r.Ｏ全角 != null && r.Ｏ全角 === r.O半角, r);
  ok('🔴 PDFの列で切れた品名も、前から当てて拾う', r.切れ != null, r.切れ);
  ok('🔴 覚えに無い品名は null（それらしい数を作らない）', r.無い === null, r.無い);
  ok('🧰 「この作業なら、ふつう一緒に出る部品」も持っている', r.組 === true, r.組);
}
