...
console.log('\n── ⑤ 🔴 画面の決めごと（車検予定の表）──');
{
  ...
  /* 🔴 v2.56.0 予約詳細は**物差し（pitShakenApply）に寄せた**＝切り方も向こう1本になった。
     ⚠ だからここで見るのは「自分で切っていないこと」＋「物差しへ渡していること」。
        写しが戻ってきたら（自分で切り始めたら）この見張りが赤くなる。 */
  ok('🔴 予約詳細は切り方を持たない（物差し1本に寄せた）',
     !/slice\(0,120\)/.test(cv.replace(/\s/g, '')) && /pitShakenApply\(s, kind,/.test(cv));
}
