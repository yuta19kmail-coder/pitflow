...
console.log('\n── ⑤ 入庫＝社内区分「代車」のカードが点検待ちで起きる ──');
{
  ...
  /* 🔗 v2.62.0（ゆうた指定 2026-09-05）**ナンバーで引き当てるのをやめた。**
     ...
     🏢🔴 v2.80.0（ゆうた報告 2026-09-07）**お客様欄の名前はもう上書きしない。**
     🗣「社用車の方、予約カードの顧客名が自社車両になってない。本当の顧客名が出ちゃってる」
        ＝ ここで本物のお客様の名前に塗り替えていたのが、ボードに顧客名が並んでいた正体。
        いまは **`customerId` だけ**を持つ（顧客ビュー・履歴への道はそのまま）。
        名札は「自社代車／自社車両」のまま＝**この試験もその時にひっくり返している。** */
  ok('🔴🔴 お客様欄の名前は「自社代車」のまま（本物の顧客名で塗り替えない）',
     card.customer === '自社代車', card.customer);
  ok('🟢 紐づけてあるお客様は控えている（顧客ビューへ行ける）', card.customerId === 'cu1', card.customerId);
