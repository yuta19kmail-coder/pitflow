...
console.log('\n── 🧭 v1.173.0 規則は2種類（抜け・矛盾／要判断）＋ 起点日 ──');
{
  ...
  /* ⚠ v2.79.0 L09（顧客控えの車種名が代車の呼び名）／
     ⚠ v2.82.0 **R01・D01**（ゆうた指定 2026-09-07「これはあってるのよ実際には。だから これでいい が必要」）
     　 ＝ どちらも規則は消していない。**1件ずつ人が見て決める**手段を置いただけ。 */
  const JUDGE = ['M04','M06','M07','F07','R01','R04','L03','L09','D01','D08','T02','T08'];   /* v2.93.0 T02 を足した */
  ...
  ok('🔴🔴 「要判断」はこの11本ちょうど（増えても減っても落ちる）',
     r.on.join() === r.want.join(), r.on);
  /* ⚠ v1.185.0 M11（売上日と実績日で月がちがう）を足したので 40 → 41。
     M11 は judge を付けていない＝**直すしかない**側。どちらを直すかは人が決めるが、
     「見て、これで合っている」で片づけてよい所見ではない（月次が実際にズレているため）。 */
  ok('🔴 残り42本は「直すしかない」', r.n - r.on.length === 42, r.n);   /* ⚠ v2.105.0 D10・T10 */
...
console.log('\n── 🗑 v1.172.2 ゆうたが「出さない」を選んでいた13本を規則表から消した ──');
{
  ...
  /* ⚠ v1.185.0 M11 を足して 48 → 49本。
     ⚠ v2.22.0 F12（入庫実績があるのに予約に戻っている）を足した。
     　 📌 最初 **F11** で書いて**この見張りに止められた**（F11 は 2026-08-25 に消した番号）。
     　 　 規則の番号を付ける時は、必ず上の GONE 一覧を避けること。
     📌 なお **M08 はコードにはあるがコメントで無効にされている**（前から）。
     　 だから「動いている本数」はコードの見た目より1本少ない。ここは**動いている数**を見ている。
     ⚠ 消した13本は1本も戻していない（上の2行が見ている）。 */
  /* ⚠ v2.79.0 L09（顧客控えの車種名が代車の呼び名になっている）を足して **51本**。
     📌 ここは長らく **49本のまま止まっていた**＝v2.62.0 で足した **L08 を数え直していなかった**（50本だった）。
     　 2026-09-07 に L09 を足した時に一緒に直した。**規則を足したら必ずここも直すこと。** */
  /* ⚠ v2.105.0 D10・T10 を足して **53本** */
  ok('残っている規則の数（いま53本）', r.n === 53, r.n);
}
{
