/* PitFlow v2.73.0 ── ✅ チェック担当者（3つ目の枠）
   ...
   ◎この試験が見張るもの
     ① 予約詳細の整備タブに3つ目の枠が出る／候補は**「メカ」の人だけ**（v2.76.0 でゆうた変更。
     　　v2.73.0 は全員だった＝**この試験もその時にひっくり返している**）
...
console.log('\n── ① 予約詳細の整備タブに3つ目の枠が出る ──');
{
  ...
  /* 🔴 v2.76.0 候補は3つとも「メカ」だけ（前は チェックだけ全員だった）
  ...
  ok('🔴🔴 チェックの候補は**「メカ」だけ**（v2.76.0 ゆうた指定）', opt.chk === opt.mech, opt);
  ok('🔴 名簿に「メカ」でない人が居る見本でやっている（全員だと差が出ない）', opt.all > opt.mech, opt);
  ok('🔴 3つの役で候補が同じ（例外はもう無い）', opt.chk === opt.ins && opt.ins === opt.mech, opt);
}
