...
(function(){
  ...
  function rasterPdf(){
    ensureJsPDF().then(ensureH2C).then(function(){
      var body=document.getElementById('view-sales-body'); if(!body){ alert('データがありません'); return; }
      ...
      }).catch(function(){ document.body.removeChild(wrap); alert('PDF出力に失敗しました。'); });
    }).catch(function(){ alert('PDFライブラリの読込に失敗しました（オフライン等）。'); });
  }
