...
console.log('\n── ② 管理者には「編集」が出る（押すまでは記録のまま） ──');
{
  ...
  /* ✂️ v2.13.1 「支払い（現金・カード…）」は丸ごと外した（ゆうた 2026-08-25）。
     ＝ 選ぶ所が無くなったので、記録を直す所にも出ない。 */
  ok('洗車・お礼LINE・車販依頼も直せる',
     ['洗車', '洗車備考', 'お礼LINE', '車販依頼', '依頼メモ'].every(k => r.rows.indexOf(k) >= 0), r.rows);
  ok('✂️ 支払いはもう出ない', r.rows.indexOf('支払い') < 0, r.rows);
