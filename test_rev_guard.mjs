...
/* =====================================================================
   ⑦ 🔴🔴 **カードに書いているアプリが、全部 版番号を足しているか**（横断の見張り）
   ---------------------------------------------------------------------
   ◎なぜ要るか（2026-08-29・実際に見落とした）
     「書き手は PitFlow と CarFlow の2つだけ」と調べて言い切ったが、**間違いだった。**
     🗣 ゆうた「MHSは当日ビューから入庫返車、車検のOKなどをやっているがそれは大丈夫？」
     → MHS も書いていた。**js フォルダしか見ておらず、index.html（812KB・全部入り）を見落としていた。**
     このまま出していたら、MHS の当日ビューから**入庫・返車・車検のOKが全部できなくなっていた。**
   ◎だからここで機械に見張らせる
     🔴 `pitCards` に **書いている**（set / update）ファイルを全部見つけて、
        そのすぐそばに **版番号（rev）を足している**かを確かめる。
     ＝ 新しく書き手が増えた時に、**その場で赤くなる。**
   ⚠ 隣のアプリのフォルダが無い時（見張りだけコピーした所で走らせた時）は**飛ばす**。
      無いものを赤にすると、関係ない所で落ちて誰も直せない（test_custform.mjs で実際に起きた）。
   ===================================================================== */
console.log('\n── 🔴🔴 ⑦カードに書いているアプリが、全部 版番号を足しているか ──');
{
  const 相手 = [
    { 名: 'MHS',     道: ['../../MHS/index.html'] },
    { 名: 'CarFlow', 道: ['../../CarFlow/carflow/js/pit-embed.js'] }
  ];
  let 見た = 0;
  相手.forEach(a => {
    const f = a.道.map(x => path.join(process.cwd(), x)).find(x => fs.existsSync(x));
    if (!f) { console.log('  ⏭ ' + a.名 + ' のファイルがこのフォルダには無いので飛ばした'); return; }
    見た++;
    const src = fs.readFileSync(f, 'utf8');
    /* 書いている所（.update( / .set( ）が pitCards の入れ物に対して在るか */
    const 書く = /\.update\(|\.set\(/.test(src);
    ok(a.名 + ' はカードに書いている（前提の確認）', 書く);
    ok('🔴🔴 ' + a.名 + ' が版番号を足している（increment）',
       /(rev\s*[:=]\s*|\.rev\s*=\s*)firebase\.firestore\.FieldValue\.increment\(1\)/.test(src),   /* `rev:` でも `patch.rev =` でも可 */
       a.名 + ' に rev の increment が見つからない');
  });
  /* PitFlow 自身は上の①〜⑥で見ている */
  const 素 = JS('db-pit.js');
  ok('🔴 PitFlow 自身も版番号を足している', /body\.rev\s*=/.test(素));
  if (見た === 0) console.log('  （隣のアプリが1つも無かった＝この場所では横断の見張りは効いていない）');
}
