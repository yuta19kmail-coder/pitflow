...
console.log('\n── ⑤ 法定費用は売上でも粗利でもない ──');
{
  const r = await p.evaluate(() => {
    ...
    return { hou: /法定費用（非課税）/.test(h), no: /売上にも粗利にも入りません/.test(h),
             ...
             bill: /112,608/.test(h),
             抜き: /売上（税抜）/.test(h), 税込: /売上（税込）/.test(h) && /58,558/.test(h),
             請求: /伝票計（請求）/.test(h),
             head: /【車検整備】/.test(h) };
  });
  ok('🧾 法定費用は別の欄に出る', r.hou === true, r);
  /* ✂️ v2.11.1（ゆうた「この辺りも要らない」）説明の2文を消した。
     🔴 消したことを見張る＝また誰かが足したら赤くする（文言は減らす、が全アプリ共通ルール）。 */
  ok('✂️ v2.11.1 「売上にも粗利にも入りません」の説明は出さない', r.no === false, r);
  ok('💰 粗利は売上−原価（法定費用は入っていない）', r.sum === true, r);
  /* 💴 v2.12.2（ゆうた「左上の計に（抜き）」「下の計の下に税込み計」） */
  ok('💴 v2.12.2 足は 売上（税抜）→ 売上（税込）→ 伝票計（請求）',
     r.抜き && r.税込 && r.請求, r);
...
{
  const r = await p.evaluate(() => {
    /* 🚗 v2.9.2 以降、**ナンバーが読めない組は先に 'plateNG'（人が見て確かめて）**になる。
       ＝ 車種で見るところまで進むのは「**両方に本物のナンバーがある**」時だけ。
       ⚠ 前のこの型はナンバーを1つも持っておらず、車種の判定に**一度も届いていなかった**
          （2026-08-25 に気づいた。ずっと赤だった）。本物のナンバーを持たせる。 */
    const P = '習志野 300 あ 1234';
    const mk = (sv, pv, sc, pc) => ({ soft:{ 車体番号:sv, 車種:sc, ナンバー:P, 顧客名:'高橋' },
                                      pit: { 車体番号:pv, 車種:pc, ナンバー:P, 顧客名:'まったく別の人' } });
    return {
